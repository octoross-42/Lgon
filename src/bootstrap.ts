...
import { FlowRunner } from "messagingFlows/model/FlowRunner.js";
...
export async function bootstrap(): Promise<void>
{
	...
	const gameStore: GameStore = new GameStore(userStore, logger);

	const flowRunner = new FlowRunner(bot.messenger, gameStore, userStore, logger);
	const lgon: LgonContext = await LgonContext.create(gameStore, userStore, flowRunner, logger);
